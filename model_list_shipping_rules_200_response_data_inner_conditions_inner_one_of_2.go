/*
Zippendo Public API

Public API documentation for Zippendo. Authenticate using your API token (Bearer token prefixed with zipp_).

API version: 1.0.0
Contact: support@zippendo.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package zippendo

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ListShippingRules200ResponseDataInnerConditionsInnerOneOf2 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ListShippingRules200ResponseDataInnerConditionsInnerOneOf2{}

// ListShippingRules200ResponseDataInnerConditionsInnerOneOf2 struct for ListShippingRules200ResponseDataInnerConditionsInnerOneOf2
type ListShippingRules200ResponseDataInnerConditionsInnerOneOf2 struct {
	// Condition type discriminator
	Type string `json:"type"`
	// Whether to compare against subtotal (before discounts) or total (after discounts)
	PriceType string `json:"priceType"`
	// Minimum cart value (inclusive)
	Min float32 `json:"min"`
	// Maximum cart value (inclusive)
	Max float32 `json:"max"`
	// Shipping price when condition matches
	ShippingPrice float32 `json:"shippingPrice"`
	// ISO 4217 currency code
	Currency string `json:"currency"`
}

type _ListShippingRules200ResponseDataInnerConditionsInnerOneOf2 ListShippingRules200ResponseDataInnerConditionsInnerOneOf2

// NewListShippingRules200ResponseDataInnerConditionsInnerOneOf2 instantiates a new ListShippingRules200ResponseDataInnerConditionsInnerOneOf2 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewListShippingRules200ResponseDataInnerConditionsInnerOneOf2(type_ string, priceType string, min float32, max float32, shippingPrice float32, currency string) *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2 {
	this := ListShippingRules200ResponseDataInnerConditionsInnerOneOf2{}
	this.Type = type_
	this.PriceType = priceType
	this.Min = min
	this.Max = max
	this.ShippingPrice = shippingPrice
	this.Currency = currency
	return &this
}

// NewListShippingRules200ResponseDataInnerConditionsInnerOneOf2WithDefaults instantiates a new ListShippingRules200ResponseDataInnerConditionsInnerOneOf2 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewListShippingRules200ResponseDataInnerConditionsInnerOneOf2WithDefaults() *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2 {
	this := ListShippingRules200ResponseDataInnerConditionsInnerOneOf2{}
	var priceType string = "total"
	this.PriceType = priceType
	return &this
}

// GetType returns the Type field value
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) GetType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) GetTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) SetType(v string) {
	o.Type = v
}

// GetPriceType returns the PriceType field value
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) GetPriceType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.PriceType
}

// GetPriceTypeOk returns a tuple with the PriceType field value
// and a boolean to check if the value has been set.
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) GetPriceTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.PriceType, true
}

// SetPriceType sets field value
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) SetPriceType(v string) {
	o.PriceType = v
}

// GetMin returns the Min field value
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) GetMin() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.Min
}

// GetMinOk returns a tuple with the Min field value
// and a boolean to check if the value has been set.
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) GetMinOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Min, true
}

// SetMin sets field value
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) SetMin(v float32) {
	o.Min = v
}

// GetMax returns the Max field value
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) GetMax() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.Max
}

// GetMaxOk returns a tuple with the Max field value
// and a boolean to check if the value has been set.
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) GetMaxOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Max, true
}

// SetMax sets field value
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) SetMax(v float32) {
	o.Max = v
}

// GetShippingPrice returns the ShippingPrice field value
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) GetShippingPrice() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.ShippingPrice
}

// GetShippingPriceOk returns a tuple with the ShippingPrice field value
// and a boolean to check if the value has been set.
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) GetShippingPriceOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ShippingPrice, true
}

// SetShippingPrice sets field value
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) SetShippingPrice(v float32) {
	o.ShippingPrice = v
}

// GetCurrency returns the Currency field value
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) GetCurrency() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Currency
}

// GetCurrencyOk returns a tuple with the Currency field value
// and a boolean to check if the value has been set.
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) GetCurrencyOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Currency, true
}

// SetCurrency sets field value
func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) SetCurrency(v string) {
	o.Currency = v
}

func (o ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["type"] = o.Type
	toSerialize["priceType"] = o.PriceType
	toSerialize["min"] = o.Min
	toSerialize["max"] = o.Max
	toSerialize["shippingPrice"] = o.ShippingPrice
	toSerialize["currency"] = o.Currency
	return toSerialize, nil
}

func (o *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"type",
		"priceType",
		"min",
		"max",
		"shippingPrice",
		"currency",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varListShippingRules200ResponseDataInnerConditionsInnerOneOf2 := _ListShippingRules200ResponseDataInnerConditionsInnerOneOf2{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varListShippingRules200ResponseDataInnerConditionsInnerOneOf2)

	if err != nil {
		return err
	}

	*o = ListShippingRules200ResponseDataInnerConditionsInnerOneOf2(varListShippingRules200ResponseDataInnerConditionsInnerOneOf2)

	return err
}

type NullableListShippingRules200ResponseDataInnerConditionsInnerOneOf2 struct {
	value *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2
	isSet bool
}

func (v NullableListShippingRules200ResponseDataInnerConditionsInnerOneOf2) Get() *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2 {
	return v.value
}

func (v *NullableListShippingRules200ResponseDataInnerConditionsInnerOneOf2) Set(val *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) {
	v.value = val
	v.isSet = true
}

func (v NullableListShippingRules200ResponseDataInnerConditionsInnerOneOf2) IsSet() bool {
	return v.isSet
}

func (v *NullableListShippingRules200ResponseDataInnerConditionsInnerOneOf2) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableListShippingRules200ResponseDataInnerConditionsInnerOneOf2(val *ListShippingRules200ResponseDataInnerConditionsInnerOneOf2) *NullableListShippingRules200ResponseDataInnerConditionsInnerOneOf2 {
	return &NullableListShippingRules200ResponseDataInnerConditionsInnerOneOf2{value: val, isSet: true}
}

func (v NullableListShippingRules200ResponseDataInnerConditionsInnerOneOf2) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableListShippingRules200ResponseDataInnerConditionsInnerOneOf2) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


