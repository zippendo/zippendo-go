...
> ListShippingRules200Response ListShippingRules(ctx, orgId).Page(page).Limit(limit).BrandId(brandId).BrandScope(brandScope).CarrierId(carrierId).Search(search).Execute()
...
func main() {
	...
	carrierId := "carr_01HZX9K2QF" // string | Filter by carrier. (optional)
	search := "Home delivery" // string | Search by rule name. (optional)

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.RulesAPI.ListShippingRules(context.Background(), orgId).Page(page).Limit(limit).BrandId(brandId).BrandScope(brandScope).CarrierId(carrierId).Search(search).Execute()
	if err != nil {
