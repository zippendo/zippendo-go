/*
Zippendo Public API

Public API documentation for Zippendo. Authenticate using your API token (Bearer token prefixed with zipp_).  **Brands (sub-accounts).** An organization can be split into brands, each keeping its own orders, shipments and configuration separate. There are two ways to scope requests to one brand, and NEITHER changes any request body:  1. **Bind the token.** Create an API token with a `brandId` and every request it makes is confined    to that brand — reads filtered, writes stamped. This is the recommended way to give a single    brand's team its own credential. 2. **Send the `X-Zippendo-Brand` header.** An organization-wide token can scope an individual    request by sending the brand's id or slug in this header. Most SDKs let you set it once on the    client so every call inherits it.  A brand-bound token that receives an `X-Zippendo-Brand` header naming a different brand is rejected with `403 BRAND_ACCESS_DENIED` — the binding is never widened. Omit both and requests cover the whole organization, which is the behaviour of every existing token.  Records that belong to no brand carry `brandId: null`. Configuration (carriers, shipping rules, addresses) with a null brand is organization-wide and remains visible inside every brand; orders and shipments with a null brand are only visible organization-wide.

API version: 1.0.0
Contact: support@zippendo.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package zippendo

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the ListOrgWebhookDeliveries200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ListOrgWebhookDeliveries200Response{}

// ListOrgWebhookDeliveries200Response struct for ListOrgWebhookDeliveries200Response
type ListOrgWebhookDeliveries200Response struct {
	// Page of results
	Data []ListOrgWebhookDeliveries200ResponseDataInner `json:"data"`
	// Total matching items across all pages
	Total float32 `json:"total"`
	// Current page number (1-based)
	Page float32 `json:"page"`
	// Items per page
	Limit float32 `json:"limit"`
	// Total number of pages
	TotalPages float32 `json:"totalPages"`
}

type _ListOrgWebhookDeliveries200Response ListOrgWebhookDeliveries200Response

// NewListOrgWebhookDeliveries200Response instantiates a new ListOrgWebhookDeliveries200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewListOrgWebhookDeliveries200Response(data []ListOrgWebhookDeliveries200ResponseDataInner, total float32, page float32, limit float32, totalPages float32) *ListOrgWebhookDeliveries200Response {
	this := ListOrgWebhookDeliveries200Response{}
	this.Data = data
	this.Total = total
	this.Page = page
	this.Limit = limit
	this.TotalPages = totalPages
	return &this
}

// NewListOrgWebhookDeliveries200ResponseWithDefaults instantiates a new ListOrgWebhookDeliveries200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewListOrgWebhookDeliveries200ResponseWithDefaults() *ListOrgWebhookDeliveries200Response {
	this := ListOrgWebhookDeliveries200Response{}
	return &this
}

// GetData returns the Data field value
func (o *ListOrgWebhookDeliveries200Response) GetData() []ListOrgWebhookDeliveries200ResponseDataInner {
	if o == nil {
		var ret []ListOrgWebhookDeliveries200ResponseDataInner
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *ListOrgWebhookDeliveries200Response) GetDataOk() ([]ListOrgWebhookDeliveries200ResponseDataInner, bool) {
	if o == nil {
		return nil, false
	}
	return o.Data, true
}

// SetData sets field value
func (o *ListOrgWebhookDeliveries200Response) SetData(v []ListOrgWebhookDeliveries200ResponseDataInner) {
	o.Data = v
}

// GetTotal returns the Total field value
func (o *ListOrgWebhookDeliveries200Response) GetTotal() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.Total
}

// GetTotalOk returns a tuple with the Total field value
// and a boolean to check if the value has been set.
func (o *ListOrgWebhookDeliveries200Response) GetTotalOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Total, true
}

// SetTotal sets field value
func (o *ListOrgWebhookDeliveries200Response) SetTotal(v float32) {
	o.Total = v
}

// GetPage returns the Page field value
func (o *ListOrgWebhookDeliveries200Response) GetPage() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.Page
}

// GetPageOk returns a tuple with the Page field value
// and a boolean to check if the value has been set.
func (o *ListOrgWebhookDeliveries200Response) GetPageOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Page, true
}

// SetPage sets field value
func (o *ListOrgWebhookDeliveries200Response) SetPage(v float32) {
	o.Page = v
}

// GetLimit returns the Limit field value
func (o *ListOrgWebhookDeliveries200Response) GetLimit() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.Limit
}

// GetLimitOk returns a tuple with the Limit field value
// and a boolean to check if the value has been set.
func (o *ListOrgWebhookDeliveries200Response) GetLimitOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Limit, true
}

// SetLimit sets field value
func (o *ListOrgWebhookDeliveries200Response) SetLimit(v float32) {
	o.Limit = v
}

// GetTotalPages returns the TotalPages field value
func (o *ListOrgWebhookDeliveries200Response) GetTotalPages() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.TotalPages
}

// GetTotalPagesOk returns a tuple with the TotalPages field value
// and a boolean to check if the value has been set.
func (o *ListOrgWebhookDeliveries200Response) GetTotalPagesOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TotalPages, true
}

// SetTotalPages sets field value
func (o *ListOrgWebhookDeliveries200Response) SetTotalPages(v float32) {
	o.TotalPages = v
}

func (o ListOrgWebhookDeliveries200Response) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ListOrgWebhookDeliveries200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	toSerialize["total"] = o.Total
	toSerialize["page"] = o.Page
	toSerialize["limit"] = o.Limit
	toSerialize["totalPages"] = o.TotalPages
	return toSerialize, nil
}

func (o *ListOrgWebhookDeliveries200Response) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"data",
		"total",
		"page",
		"limit",
		"totalPages",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varListOrgWebhookDeliveries200Response := _ListOrgWebhookDeliveries200Response{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varListOrgWebhookDeliveries200Response)

	if err != nil {
		return err
	}

	*o = ListOrgWebhookDeliveries200Response(varListOrgWebhookDeliveries200Response)

	return err
}

type NullableListOrgWebhookDeliveries200Response struct {
	value *ListOrgWebhookDeliveries200Response
	isSet bool
}

func (v NullableListOrgWebhookDeliveries200Response) Get() *ListOrgWebhookDeliveries200Response {
	return v.value
}

func (v *NullableListOrgWebhookDeliveries200Response) Set(val *ListOrgWebhookDeliveries200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableListOrgWebhookDeliveries200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableListOrgWebhookDeliveries200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableListOrgWebhookDeliveries200Response(val *ListOrgWebhookDeliveries200Response) *NullableListOrgWebhookDeliveries200Response {
	return &NullableListOrgWebhookDeliveries200Response{value: val, isSet: true}
}

func (v NullableListOrgWebhookDeliveries200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableListOrgWebhookDeliveries200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


